...
<script>
import SearchSelector from '@/views/Search/SearchSelector.vue'
...
export default {
  ...
  methods: {
    // 向服务器发起请求获取 search 模块数据（根据参数不同，返回不同数据）
    getData() {
    ...
    trademarkInfo(trademark) {
      // console.log('search', trademark)
      // 整理品牌字段的参数， ID：品牌名称
      this.searchParams.trademark = `${trademark.tmId}:${trademark.tmName}`
      // 再次发起请求获取 search 模块列表数据
      this.getData()
    },
    // 删除品牌
    removeTrademark() {
    ...
    changeOrder(flag) {
      if (flag === this.searchParams.order.split(':')[0]) {
        this.searchParams.order = `${flag}:${this.isAsc ? 'desc' : 'asc'}`
      } else {
